...
function MyEnrolledCourses() {
    ...
    const {courseData} = useSelector(state=>state.course)  // yeh add karo
    ...
    const enrolledCourses = courseData?.filter(course =>
        userData?.enrolledCourses?.some(id =>
            (typeof id === 'string' ? id : id._id).toString() === course._id.toString()
        )
    )
                ...
                {enrolledCourses?.map((course, index)=>(
                    <div key={index} className='bg-white rounded-2xl shadow-md overflow-hidden border'>
                        ...
                        <div className='p-4'>
                            ...
                            <h1 className='px-[10px] text-center py-[10px] border-2 bg-black border-black text-white rounded-[10px] text-[15px] font-light cursor-pointer mt-[10px] hover:bg-gray-600'
